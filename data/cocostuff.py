...
class COCOStuff164k(torch.utils.data.Dataset):
    def __init__(self, path_list = 'dataset/cocostuff164k/' , transform = None, data_set = 'train2017', seed = None, aug = True,
                label_convert = False, img_size=768, return_label = True,  
                interpolation=Image.BILINEAR, color_pallete = 'city', norm_ = False):
        """
        ...
        if data_set in ['train2017', 'val2017']:
            self.tr_val = data_set # 'training' or 'validation'
        else: raise Exception("data_set should be 'train2017' or 'val2017'")
        ...
        self.aug_weights = [1, 0.3, 0.3, 0.2, 0.2, 0.05, 0.05, 0.02, 0.02]
        if img_size:
    ...
    def __getitem__(self, index):
        ...
        mask = Image.fromarray((np.array(mask)+1)%256) 
